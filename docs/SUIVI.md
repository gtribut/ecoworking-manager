...
>
> **Dernière mise à jour : 2026-09-17 — **C14 refonte UI portail TERMINÉ** (U0→U5 + retours de recette C14.7/C14.8 mergés, dernier `afacc07`) : 668 Pest + 288 Vitest + e2e 52/52 (26 audits axe clair/sombre) + Lighthouse a11y 100 ; recette §3 à rejouer (todo_guillaume §C14).** Précédent : 2026-09-16 — C14 démarré : U0 (docs) livré ; suite U1→U5 en session orchestrateur. 2026-09-16 — C14 refonte UI portail planifié (`docs/refonte_ui/`), recette §3.1 faite, §3.2+ suspendue jusqu'à C14. 2026-09-13 (soir) — C13.6 lot B mergé, lot A livré (branche `feature/portail-lot-a`, à relire + rejouer Playwright). Reprise 12/09 après 2 mois. MVP COMPLET ✅ depuis le 03/07 (444 Pest + 72 Vitest + 26 e2e). Chantier ouvert : C13 recette manuelle (`docs/recette.md` + `DemoSeeder`, livrés le 12/09, validés le 13/09 : 446 Pest verts). Ensuite : corrections issues de la recette, puis V1.5 (Clever Cloud, import Cosoft) — cf. `todo_guillaume.md`.
...
| C13.7 | Doc de présentation illustrée (`docs/presentation/`) | ✅ | 22/09 : `README.md` (fonctionnel + workflows + « sous le capot ») + 30 captures (15 portail dont 3 mobile, 15 admin) + PDF 26 p. Script `scripts/presentation/build.sh` : base dédiée `presentation` (DemoSeeder), Playwright `portal-spa/presentation/` avec login admin 2FA (secret TOTP de démo), PDF via marked + Chromium. Bug trouvé en capturant → R-09 corrigé (filtre « Entité » des listes Documents entités / Contacts / Profils membres : 500 dès qu'un particulier existe) + test `CompanyFilterIndividualTest` |
...
| C14.3 | U3 agenda salles FullCalendar v7 (repli v6), alternative liste + bouton | ✅ | Opus — mergé 16/09 (`654395b`) : `@fullcalendar/react` 7.1 (paquet unique, plugins en sous-chemins, thème classic, `temporal-polyfill`) stable avec React 19 / Vite 8 → **pas de repli v6** ; `RoomsCalendar` réécrit (toolbar shadcn, chips de salles, popover, `eventContent` 3 lignes, `--fc-classic-*` remappées, `.fc` posée par l'app pour l'exclusion axe D4), module pur `calendarEvents.ts`, mini-mois shadcn `Calendar` (react-day-picker 10 + date-fns, libellés FR), champ Salle dans `BookingDialog`, `MyBookingsList` = alternative accessible ; e2e réécrits via « Nouvelle réservation » + 2 tests de glisser-créer (membre, external). Review Opus soldée (dépendance parasite `cn` retirée, popover fermé au scroll, nom de salle dans les blocs). 663 Pest + 276 Vitest + e2e 26/26. ⏸️ Docs à corriger en U5 : ADR-0013/PRD §3.5.2-3.5.4/BRIEF (packaging v7 réel) |
| C14.4 | U4a pages : dashboard bento, factures DataTable, profil onglets | ✅ | Sonnet — mergé 16/09 (`2892be8`) : `DashboardKpiGrid`/`KpiTile` (prochaine résa, mon bureau via `/api/profile`, tickets, documents à valider — tuile masquée si rien à valider (R-06), dernière facture ; gating inchangé), blocs résas/actus/factures restylés, liens rapides supprimés ; factures en `@tanstack/react-table` 8 (tri/pagination serveur conservés, `aria-sort`, focus après tri) ; profil en `Tabs` Profil/Compte/Préférences/Entreprise (`?tab=`, bascule sur l'onglet en erreur). Review Opus soldée (date des résas en sr-only, meta de tri namespacé, requête dupliquée). 663 Pest + 285 Vitest + e2e 26/26. ⏸️ PRD §3.3.2 à ré-acter en U5 (validation des documents depuis `/documents`, plus depuis l'accueil) |
...
| C14.6 | U5 finition : axe clair/sombre, Lighthouse, recette §3 mise à jour, docs | ✅ | Sonnet — mergé 16/09 (`5946b40`) : utilitaire `text-link` (styles.css, remplace ~20 occurrences de `text-brand-700 dark:text-brand-300` + `Button`/`Badge` variant `link`), contraste `text-destructive` corrigé (`dark:text-red-300` sur `Badge`/`DropdownMenuItem` destructifs), pastille active `DirectoryTabs` (`bg-card` + `ring-foreground/10`) ; passe axe complète clair+sombre (`e2e/a11y-sweep.spec.ts`, 26 tests neufs : profil 4 onglets, présence, notifications, tickets, plan avec panneau ouvert, mot de passe oublié, 3 pages publiques, bottom nav Sheet « Plus », mobile 390 px sans scroll horizontal ×4, zoom 200 %, `prefers-reduced-motion`) — **zéro violation** ; Lighthouse a11y **100/100** sur login/accueil/factures/profil/annuaire, clair et sombre (seul signal : règle axe expérimentale poids 0, non corrigée, cf. todo) ; nettoyage `calendar.ts` (7 exports morts retirés), justification documentée du doublon `Avatar`/`ui/avatar`, désynchronisation `useThemeSelection` corrigée (dérivée de la query `authQueryKey`, plus de `useState` par instance) ; mineurs F (listes Markdown, sr-only « Valider », `last:pb-0`, grille KPI `auto-fit`, ancres focusables, doublon « Nous contacter » retiré) ; bundle : chunk principal 660→520 kB gzip 196→156 kB (lazy des pages restantes + extraction `announcements/format.ts`) ; ADR-0013/PRD/BRIEF/recette §3/todo_guillaume mis à jour. 663 Pest, 285 Vitest, e2e 52+26=78/78, Pint clean, Biome clean, tsc clean. Point ouvert : titres de section `text-lg` non homogénéisés (noté, risque/diff jugé disproportionné pour une finition) |
| C14.7 | Retours de recette du 16/09 (post-U5) | ✅ | Sonnet — mergé 16/09 (`cbdc39d`) : variantes Tailwind `data-horizontal`/`data-vertical` manquantes depuis l'init shadcn (onglets du profil empilés à gauche ; touchait aussi Separator/ScrollArea/ToggleGroup), « Mes prochaines réservations » sous l'agenda dans la colonne calendrier, hauteur des heures +50 % (`height` + `expandRows`, pas de `slotHeight` en v7), vue Mois retirée (ADR-0013, PRD §3.5.2, recette §3.4 alignés). 663 Pest + 286 Vitest + e2e 52/52 |
| C14.8 | Présence résidents 7 j/7 (retour recette 17/09) | ✅ | Opus — mergé 17/09 (`afacc07`) : garde `FrenchHolidays` retirée de `PresenceService::presentOn()` (présent tous les jours, week-ends et fériés compris, sauf absence déclarée), bandeau « jour non ouvré » du plan portail retiré, encart admin « Occupation du jour » reformulé ; gardes jours ouvrés nomades/salles intactes ; PRD §3.7.2/§3.7.3/§6.1 ré-actés, recette §3.3/§3.10/§4.8. 668 Pest + 288 Vitest + e2e 52/52 |
