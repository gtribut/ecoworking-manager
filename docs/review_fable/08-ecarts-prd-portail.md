...
| Hover/clic résa des autres = prénom + nom + entité + libellé (Q4) | ✅ *soldé 13/09 (lot A, `e14306c`)* | `occupant` + `label` exposés ; infobulle au survol **et** au focus clavier |
...
|---|---|---|---|
| A — Calendrier salles | ✅ livré 13/09 (branche `feature/portail-lot-a`) | `241405a`, `6f6f512`, `e14306c` | 501 Pest / 121 Vitest verts (Playwright non exécuté : specs `bookings`/`a11y` adaptées, à rejouer). Corrigé au passage : `BookingPolicy` comparait `starts_at->isFuture()` en PHP → une résa **déjà commencée** restait annulable ~2 h (piège timezone). Reste **hors lot** : vérification d'**abonnement actif** (§3.5.3) non implémentée ; décalage de fuseau à l'écriture (cf. « Écarts hors lot ») |
