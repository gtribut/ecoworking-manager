...
describe('DashboardInvoices (R-05, PRD §3.3.2)', () => {
  it('affiche 3 factures max avec numéro, date, statut et lien PDF, puis le lien « voir toutes »', async () => {
    server.use(
    ...
    expect(screen.getByText('PDF indisponible')).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /PDF de la facture EW-2026-00001/ })).toHaveAttribute(
      'href',
      '/api/invoices/1/pdf',
    )
    expect(screen.getByRole('link', { name: 'Voir toutes mes factures →' })).toHaveAttribute(
      'href',
      '/invoices',
    )
  })
